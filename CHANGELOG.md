...
- A Box upload whose name is already taken now writes a new revision of the existing file. Box refuses a create whose name is taken, and the sync uploads the changes file before the version file, so a version upload that failed after its changes succeeded used to hit that conflict on every retry and the version could never land.
